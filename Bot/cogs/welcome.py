        ...
        if await self.redis.hget("{}:Config:Cogs".format(member.guild.id),"welcome") == "on":
            ...
            try:
                if config.get("enable_message") == "on":
                    if config.get("whisper") == "on":
                        ...
                        msg = self.bot.get_channel(int(config["channel"])).send(config["message"].format(user=member.name,guild=member.guild,user_mention=member.mention))
            ...
            if config.get("role") == "on":
                try:
                    ...
                    role_obj=[]
                    for x in role_list:
                        role_obj.append(discord.utils.get(member.guild.roles,id=x))
